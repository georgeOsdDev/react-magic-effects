...
export default (progress) => {
  ...
  if (progress < 50){
    ...
    transform = `scale(${utils.calc(1, 1.1, progress)}, ${utils.calc(1, 1.1, progress)}) translateY(${utils.calc(900, 0, progress)}%)`;
  } else if (progress < 60){
    progress = (progress - 50)*10;
    opacity = 1;
    transform = `scale(${utils.calc(1.1, 1, progress)}, ${utils.calc(1.1, 1, progress)}) translateY(0)`;
  } else if (progress < 70){
